...
#        return items  
    ...
    def _explore(self):
        ...
        (uuids, base_items) = self._get_base_cache(self.definition.base_definition, params)
        explorer.tasks.send_jobs_with_params.apply_async(kwargs={'base_models': base_items}, args=[self.definition, uuids, self.root, params, self.distance, self.material, self.page_size, 'iterate', self.text])
        
        self.root.selected=True
        self.root.save()
        return self._make_result(uuids, materials, [self.text for i in range(len(uuids))])
        
    
    def render_materials_old(self, materials, parent_id, definition_id, text):
        if parent_id == None:
            root = None
