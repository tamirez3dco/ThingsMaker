...
$.fn.exists = function() {
	...
	$.widget("db.jWizard", {
		...
		_create : function() {
			...
			var textToScreen = "";
			if (typeof tempTextParam != "undefined"){
				if (tempTextParam != null){
					textToScreen = decodeURIComponent(tempTextParam);
				}
			}

			this._userText = textToScreen;
		...
		_openTip1: function() {
			...
			var top = $(".create-image-container:visible").offset().top-$(window).scrollTop()
		...
		_openTip3: function() {
			...
			var top = $(".create-image-container:visible").offset().top-$(window).scrollTop()+48;
		...
		_addImagesToStep : function(step, stepidx) {
			...
			for(var i = 0; i < data.length; i++) {
				...
				var id = stepidx + '-' + i;
				var html = '<div class="'+wrpClass+'"><img class="loader" src="http://ez3d_statics2.s3.amazonaws.com/sitestatic/img/loading141.gif"/><div class="wrap-in"><div class="'+expClass+'" id="explorer-image-' + id + '"><button class="explorer-image-button" type="button">Make It</button></div></div></div>';
		...
		_replaceShareButtons : function(image_url, product_url) {
			$(".product-share-popup").html('<div class="first">Share It</div><div class="second"></div>');
			var services = [
